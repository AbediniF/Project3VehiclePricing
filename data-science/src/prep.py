...
def parse_args():
    ...
    parser.add_argument("--train_dataset", type=str, help="Path to train dataset")  # Specify the type for train data (str)
    parser.add_argument("--test_dataset", type=str, help="Path to test dataset")  # Specify the type for test data (str)
...
def main(args):  # Write the function name for the main data preparation logic
    ...
    # create directories to save train and test datasets
    os.makedirs(args.train_dataset, exist_ok=True)
    os.makedirs(args.test_dataset, exist_ok=True)
    train_df.to_csv(os.path.join(args.train_dataset,"train_dataset.csv"), index=False)
    test_df.to_csv(os.path.join(args.test_dataset,"test_dataset.csv"), index=False)
...
if __name__ == "__main__":
    ...
    lines = [
        ...
        f"Train dataset output path: {args.train_dataset}",  # Print the train_data path
        f"Test dataset path: {args.test_dataset}",  # Print the test_data path
